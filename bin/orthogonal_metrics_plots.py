...
def plot_combined_cohort_orthogonal_summary(
    ...
    # Rationale: at small cohort sizes, two sequences in the same tier
    # can sit next to each other where one has clean confidence and the
    # other has a single failed confidence threshold — promoting the
    # clean one above the failing one makes the top-of-plot a defensible
    # "best of the survivors so far" reading.
    ...
    # Place the two legends side-by-side rather than stacked.  Each
    # is anchored to its own half of the figure width so the cell-fill
    # legend sits to the left of the tier-stripe legend at the same y.
    # ncol=2 keeps each legend compact (2×2 patch grid) so titles
    # don't bleed into the neighbouring block.
    legend_cells = ax.legend(
        handles=cell_handles, loc="upper left",
        bbox_to_anchor=(0.0, -0.04), ncol=2,
        fontsize=8, framealpha=0.95,
        title="Cell colours: individual-metric pass/fail",
        title_fontsize=8.5,
    )
    ax.add_artist(legend_cells)
    ax.legend(
        handles=stripe_handles, loc="upper right",
        bbox_to_anchor=(1.0, -0.04), ncol=2,
        fontsize=8, framealpha=0.95,
        title="Left stripe: per-sequence negsteer cohort outcome\n(independent of cell colours)",
        title_fontsize=8.5,
    )
