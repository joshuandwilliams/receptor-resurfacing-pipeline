...
----------------------
Standalone iteration harness for negsteer plots (Task 43).

Reads cross_sequence_summary.csv produced by a previous
``test_negative_steering`` run and re-emits the diagnostic plots in
an output directory of choice.  No GPU, no Nextflow — designed for
fast iteration of the plot code itself against cached test outputs.
...
Usage
-----
    python test_negsteer_plots.py \\
        --csv tests/negative_steering/receptor_resurfacing_results/negative_steering/cross_sequence_summary.csv \\
        --rfdiff-metrics tests/rfdiffusion/receptor_resurfacing_results/rfdiffusion/rfdiffusion_metrics.json \\
        --outdir tests/negative_steering/receptor_resurfacing_results/plots_iter

Both --csv and --rfdiff-metrics fall back to canonical test paths if
omitted.  --rfdiff-metrics is optional — when absent the mutation-
impact plot still renders, just without design-region shading.
"""
...
def load_unified_cohort(
    ...
    if verbose:
        ...
        # Count rows missing seed-count data (the tell that backfill
        # is needed for them).
...
def _resolve_csv_path(supplied: Optional[str]) -> Path:
    ...
    if fallback.exists():
        return fallback
    raise SystemExit(
        f"No --csv passed and fallback {fallback} does not exist.\n"
...
def _parse_args() -> argparse.Namespace:
    ...
    ap.add_argument("--csv", default=None,
        help="cross_sequence_summary.csv from a test_negative_steering "
             "run.  Falls back to canonical test path if omitted.")
