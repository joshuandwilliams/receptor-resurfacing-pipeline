...
--------------------------------
Standalone iteration harness for orthogonal-metrics plots (Task 44).

Reads survivors_with_orthogonal_metrics.csv produced by
test_orthogonal_metrics, plus cross_sequence_summary.csv from the
upstream test_negative_steering run.  Both are required as of
2026-04-28 — the metrics-vs-composite scatter and the combined
cohort+orthogonal summary both join on mpnn_sequence to source
upstream composite scores and cohort-wide context.
...
orthogonal_af3_vs_boltz.png
    ...
    representative_ra_eff_vs_truth_median (y).  Survivor names
    annotated with greedy 4-corner overlap avoidance.  AF3 region
    failing the af3_ra_max threshold shaded pale red — but AF3 is
...
Usage
-----
    python test_orthogonal_metrics_plots.py \\
        --survivors-csv tests/orthogonal_metrics/receptor_resurfacing_results/orthogonal_metrics/survivors_with_orthogonal_metrics.csv \\
        --cross-summary-csv tests/negative_steering/receptor_resurfacing_results/negative_steering/cross_sequence_summary.csv \\
        --outdir tests/orthogonal_metrics/receptor_resurfacing_results/plots_iter

Both --survivors-csv and --cross-summary-csv are required (each
falls back to its canonical test path when omitted).
"""
...
def _resolve_csv_path(supplied: Optional[str], fallback_rel: str,
...
def _parse_args() -> argparse.Namespace:
    ...
    ap.add_argument("--survivors-csv", default=None,
        help="survivors_with_orthogonal_metrics.csv from a "
             "test_orthogonal_metrics run.  Falls back to canonical "
             "test path if omitted.")
    ap.add_argument("--cross-summary-csv", default=None,
        help="cross_sequence_summary.csv from the upstream "
             "test_negative_steering run.  Optional — used by plots "
             "that need full upstream cohort context.")
...
def main() -> int:
    args = _parse_args()
    survivors_path = _resolve_csv_path(
        args.survivors_csv,
    ...
    if survivors_path is None:
        raise SystemExit(
            "No --survivors-csv passed and fallback does not exist.\n"
            "Run test_orthogonal_metrics first or pass --survivors-csv "
            "explicitly."
        )

    cross_summary_path = _resolve_csv_path(
        args.cross_summary_csv,
        "tests/negative_steering/receptor_resurfacing_results/"
        "negative_steering/cross_sequence_summary.csv",
        label="cross-summary-csv",
    )

    outdir = Path(args.outdir)
    outdir.mkdir(parents=True, exist_ok=True)

    survivor_rows = _load_rows(survivors_path, "survivors")
    ...
    # cross_summary is now used by two plots (metrics_vs_composite and
    # the combined cohort+orthogonal summary).  Required.
    if cross_summary_path is None:
        raise SystemExit(
            "No --cross-summary-csv passed and fallback does not exist.\n"
            "The metrics-vs-composite and combined-summary plots both "
            "need it to source composite scores and the upstream cohort.\n"
            "Run test_negative_steering first or pass --cross-summary-csv "
            "explicitly."
        )
    cross_rows = _load_rows(cross_summary_path, "cross-summary")
