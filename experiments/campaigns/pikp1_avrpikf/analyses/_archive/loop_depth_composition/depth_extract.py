...
def main():
    ...
    dssp = pd.read_csv(os.path.join(DATA, "design_secondary_structure.csv"))
    ...
    jobs = [(p, os.path.basename(p)[:-4], sequences.get(os.path.basename(p)[:-4], []))
            for p in sorted(glob.glob(os.path.join(DATA, "rfdiffusion", "design_*.pdb")))]
    jobs.append((os.path.join(DATA, "wt_reference.pdb"), "wild-type",
                 [("wt", native)]))
