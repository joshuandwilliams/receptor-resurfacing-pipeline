...
Three views of the same table, differing in how much they commit to a binning.
    ...
    composition_stack_fixed the whole composition, in fixed-width depth bins
...
composition_stack() still takes equal-occupancy bins and a wild-type overlay,
which produced three further variants of the same plot. Those were dropped
once the fixed-bin version was chosen; the arguments remain if they are wanted
again.
...
def composition_stack(designs, band, outdir, width=None, origin=BIN_ORIGIN,
    ...
    if width is None:
        ...
        ax.set_xlabel("Distance from the hairpin anchors (Å), equal-occupancy "
                      f"bins of {int(stats.n.iloc[0])} residues")
    else:
        # The bin width is readable off the tick labels, so it is not repeated.
        ax.set_xlabel("Distance from the hairpin anchors (Å)")
...
def tip_reach_by_length(designs, outdir):
    """How far the loop tip sits from the anchors, against region length.

    Not a thesis figure. Reach is almost fully determined by length, so the
    plot says only that longer loops reach further. The two numbers it gives,
    16.7 A at 10 residues and 30.0 A at 19, are quoted in the prose instead.
    Call it directly if the plot is wanted.

    The polar and charged residues sit at the tip, so the distance the tip
    reaches is the distance at which a design presents them. RFdiffusion
    sampled design region 1 between 10 and 20 residues, so that reach is not
    the same between designs. One point per designed sequence, taking the
    deepest residue of its own region.
    """
    tip = (designs.groupby(["design", "seq"])
           .agg(length=("region_length", "first"), reach=("depth", "max"))
           .reset_index())
    # Distance on x so this panel shares an axis with the composition panel.
    fig, ax = plt.subplots(figsize=(5.6, 3.6))
    ax.scatter(tip.reach, tip.length, s=34, color=COLOUR_OF["Negative"],
               alpha=0.75, edgecolor="white", linewidth=0.6, zorder=3)
    med = tip.groupby("length").reach.median()
    ax.plot(med.values, med.index, lw=1.6, color=INK, zorder=2)
    ax.set_xlabel("Distance from the anchors to the loop tip (Å)")
    ax.set_ylabel("Design region 1\nlength (residues)")
    ax.set_yticks(range(10, 21, 2))
    fig.tight_layout()
    save(fig, outdir, "tip_reach_by_length")
