...
# assignment is made the same way rather than quoted from the literature. It is
# the campaign input complex; chain A is the wild-type Pikp-1 HMA and only
# chain A is kept below.
paths = sorted(glob.glob(os.path.join(RUN, "rfdiffusion", "design_*.pdb")))
paths.append(os.path.join(INPUTS, "pikp1_avrpikf_complex_ab.pdb"))

rows = []
for path in paths:
    design = os.path.basename(path).replace(".pdb", "")
    if design == "pikp1_avrpikf_complex_ab":
        design = "wild-type"
