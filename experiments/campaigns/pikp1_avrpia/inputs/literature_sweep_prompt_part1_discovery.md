...
- The campaign's own README at `experiments/campaigns/pikp1_avrpia/README.md` if present.
- Any existing complex PDB at `experiments/campaigns/pikp1_avrpia/inputs/` (if not present, note that complex assembly is an outcome of this sweep, not an input). If a complex PDB exists, determine (a) the source PDB it was assembled from, and (b) the residue-numbering convention used — both feed Part 2's structural-input decision and numbering-translation sections.
- Any existing contig file in the campaign's `inputs/` directory (if present) — it tells you which numbering convention the campaign is currently working in.
- Outputs of the parallel pikp1_* campaigns' literature sweeps in their `inputs/` directories — the Pikp-1 HMA binder-side analysis is shared, so receptor-side findings from a sibling campaign are reusable here.
...
5. Write a fetch manifest at:
   `experiments/campaigns/pikp1_avrpia/inputs/pikp1_avrpia_fetch_manifest.md`
