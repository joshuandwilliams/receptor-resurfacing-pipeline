...
You are conducting the synthesis phase of a structured literature, PDB, and UniProt sweep to inform the design of a binder-resurfacing campaign. Part 1 produced a fetch manifest at `experiments/campaigns/pikp1_avrpia/inputs/pikp1_avrpia_fetch_manifest.md` listing papers that could not be accessed online. The user has now placed the requested PDFs into `experiments/inputs/context/`. Your task here is to read those new PDFs, combine them with everything you already fetched online in Part 1, and write the full literature-sweep report.
...
- AVR-Pia is recognised natively by the rice NLR pair RGA4/RGA5 (not Pikp), and the canonical AVR-Pia binding site sits on the RGA5 HMA. There is also a published crystal structure of Pikp-HMA bound to AVR-Pia (PDB 6Q76, Varden et al. 2019). The user has indicated they suspect AVR-Pia uses a different face of the Pikp HMA than the canonical AVR-Pik binding face — Part 2's structural-input decision and contig design must verify which face the campaign is targeting and pick the alignment template accordingly. The MAX-fold effector / HMA binding mode means a ChimeraX alignment onto an appropriate published complex template should suffice, but 6G10 is *not* the right template if AVR-Pia uses a different face.
- The Pikp-1 HMA binder is shared across this campaign and the other pikp1_* campaigns; the receptor-side anchor / flex-region analysis from those campaigns is reusable here, but the binding-face determination is campaign-specific.

---

## What to read first

1. The fetch manifest at `experiments/campaigns/pikp1_avrpia/inputs/pikp1_avrpia_fetch_manifest.md` — header records source-PDB and numbering-convention determinations (or recommendations) carried forward from Part 1.
...
If the campaign's effector binds the receptor at a different face than the canonical AVR-Pik face, name the alignment template explicitly (e.g. "use 6Q76 chain superposition rather than 6G10") and describe the alignment recipe.
...
**Design principle: the flexible regions must contain the residues to be redesigned.** A residue inside an `A<start>-<end>` block is preserved native; a residue inside a flexible block is replaced by RFDiffusion. State explicitly which key receptor-side residues from §4 fall inside flexible regions and which fall inside anchored regions. Do not anchor on or copy any contig string from a sibling pikp1_* campaign — the contact pattern in *this* campaign's complex (especially if the effector binds a different face of the receptor) determines the flex / anchor regions.
...
Deliver the report as a single Markdown document at `experiments/campaigns/pikp1_avrpia/inputs/pikp1_avrpia_literature_sweep.md`. Use the section structure prescribed above. Use Markdown tables for structure inventories and renumbering translations. Use code blocks for contig strings. Cite references inline by author + year on first mention, with a final References section listing each citation in full at the end of the document.

Also write a sibling provenance file `pikp1_avrpia_literature_sweep.notes` recording: the date the sweep was completed, the source-PDB and numbering-convention determinations, the names of the primary papers cited, and which papers came from the context folder vs online.
